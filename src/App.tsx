...
function App() {
  ...
  const [useTacticalMode, setUseTacticalMode] = useState(true); // Default to tactical mode
  ...
  // Toggle between tactical and standard mode
  const toggleMode = () => {
    setUseTacticalMode(!useTacticalMode);
  };

  return (
    <ConversationProvider>
      {useTacticalMode ? (
        // Tactical Command Center Mode
      ...
      <div className="max-w-6xl mx-auto w-full flex-1 flex flex-col relative">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 px-4 py-3">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-2 md:mt-0 w-full md:w-auto justify-end">
              ...
              {/* Wake Word Indicator */}
              <div className="px-4 py-2 rounded-full bg-card/80 border border-border/10 shadow-md">
                <WakeWordIndicator className="" showControls={false} autoStart={true} />
              </div>
        ...
        <button
          onClick={toggleMode}
          className="fixed bottom-4 right-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-full shadow-lg transition-all z-50"
          title="Switch to Tactical Mode"
        >
          🎯 Tactical
        </button>
      </div>
