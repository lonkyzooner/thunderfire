...
export const LoginForm: React.FC<LoginFormProps> = ({ onSuccess, onRegisterClick }) => {
  ...
  const handleLogin = async () => {
    try {
      ...
      console.log('[LoginForm] Initiating Auth0 login');
      
      // Use Auth0 login
      await login();
      
      console.log('[LoginForm] Auth0 login successful');
      ...
      console.error('[LoginForm] Auth0 login failed:', err);
      handleError({
  ...
  return (
    <Card className="w-full max-w-md mx-auto">
      <CardHeader className="space-y-1">
        <CardTitle className="text-2xl font-bold text-center">Sign in to LARK</CardTitle>
