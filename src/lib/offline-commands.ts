  ...
  try {
    ...
    if (mirandaMatch) {
      ...
      if (MIRANDA_RIGHTS[language]) {
        ...
        // Note: Caching handled by context service
        return response;
      }
    }
    ...
    if (statuteMatch) {
      const statuteKey = statuteMatch[1] as keyof typeof COMMON_STATUTES;
      if (COMMON_STATUTES[statuteKey]) {
        const response: CommandResponse = {
          command: transcript,
          action: 'statute',
          parameters: { statute: statuteMatch[1] },
          executed: true,
          result: COMMON_STATUTES[statuteKey]
        };
